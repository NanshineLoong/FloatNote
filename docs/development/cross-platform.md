...
- 系统功能：捕获、辅助功能、浏览器 attribution 与部分自动化能力有 macOS 实现或权限要求；代码必须保留 `cfg(target_os = ...)` 分支和非 macOS 回退。自动划词当前仅在 macOS 启用，使用独立 listen-only event tap 与 Accessibility；`shortcut` 模式不安装全局监听。Windows 设置页明确标注 macOS 自动模式，Windows 仍以后续平台 adapter 为边界。
