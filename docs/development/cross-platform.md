...
- UI：设置窗口默认 `780 × 620`、最小 `720 × 520`，允许缩放和最大化。macOS 使用原生装饰与 Overlay 标题栏，38px 空白外壳区域可拖动并保留系统红绿灯；不抢焦点的划词弹窗通过窄 AppKit 桥接开启 `NSWindow.acceptsMouseMovedEvents`，否则 WebView 的 `:hover` 不会随纯鼠标移动更新。Windows 保留原生最小化、最大化与关闭控件，不模拟红绿灯。内容信息架构和卡片间距在两个平台一致。改动这些区域时，应在 macOS 与 Windows 各验证一次。
