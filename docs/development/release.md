...
双架构 DMG、原生 runner 架构、GitHub 权限、Draft Release 创建与资产上传只能由 GitHub Actions 最终验证。修改 Tauri bundling、resources、entitlements 或签名配置时，还应在匹配架构的 macOS 机器上额外运行 `npm run tauri build`；日常发布预检不重复这项耗时构建。
...
应用图标的可编辑母版位于 `src-tauri/icons/app-icon.svg`。修改母版后先重新生成各平台资源，并检查变更，再进行打包：
