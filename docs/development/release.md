...
FloatNote 的 macOS 预览版由 GitHub Actions 构建。推送版本标签后，工作流分别在原生 Apple Silicon 与 Intel runner 上构建 `.dmg`，使用 Developer ID Application 签名并提交 Apple 公证，最后创建 Draft、Prerelease GitHub Release。Draft 经人工检查后才会公开。
...
签名、公证或验证失败时不会上传该架构的资产。上传阶段会按资产名替换同一 Draft 中的旧版本；如果上传中途失败，Draft 可能暂时只有部分资产，此时不得人工发布，重新运行工作流后再审核。
...
确认 Release 标题、说明、两种架构的 `.dmg`/应用归档、签名公证检查和功能测试结果后，再在 GitHub 将 Draft 发布。预览阶段应保留 Prerelease 标记。
