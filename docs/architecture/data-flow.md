...
`ls/find/grep/read` 通过 FloatNote inline extension 与 Rust 的 `workspace_list` / `workspace_read` 动态访问当前 project space；具体文件名不注入 system prompt，也不启用 Pi 本地文件工具。`write` 的 create 模式只允许根目录新 piece，并以 create-only 原子提交拒绝同名竞态。网络研究仍由原有 `web_search` / `web_fetch` 实现完成，不进入本地写权限流程。
