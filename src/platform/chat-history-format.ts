...
export function deriveTitleFromFirstMessage(text: string): { title: string; titleState: ChatTitleState } {
  const trimmed = text.trim();
  const chars = [...trimmed];
  if (chars.length <= 10) return { title: trimmed || "新对话", titleState: "final" };
...
export function formatHistoryTime(timestamp: number, now = Date.now()): string {
  ...
  if (date.getFullYear() === today.getFullYear()) return `${pad2(date.getMonth() + 1)}/${pad2(date.getDate())}`;
