...
export function chatGetForScope(scope: ChatScope): Promise<ChatConversation | null> {
  return invoke<ChatConversation | null>("chat_get_for_scope", {
...
export function chatCreate(scope: ChatScope): Promise<ChatConversation> {
  return invoke<ChatConversation>("chat_create", {
    scopeType: scope.scopeType,
    scopePath: scope.scopePath,
...
export function chatListForScope(scope: ChatScope): Promise<ChatConversation[]> {
  return invoke<ChatConversation[]>("chat_list_for_scope", {
    scopeType: scope.scopeType,
    scopePath: scope.scopePath,
  });
}
...
export function chatUpdateTitle(
  conversationId: string,
  title: string,
  titleState: ChatTitleState,
): Promise<ChatConversation | null> {
  return invoke<ChatConversation | null>("chat_update_title", {
    conversationId,
    title,
    titleState,
  });
